...
//    commandList->LoadTextureFromFile(m_GraceCathedralTexture, L"Assets/Textures/UV_Test_Pattern.png");
    ...
    // Create a root signature and PSO for the skybox shaders.
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed(D3DReadFileToBlob(L"Skybox-vs.cso", &vs));
        ThrowIfFailed(D3DReadFileToBlob(L"Skybox-ps.cso", &ps));
    ...
    // Create a root signature for the HDR pipeline.
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed(D3DReadFileToBlob(L"HDR-vs.cso", &vs));
        ThrowIfFailed(D3DReadFileToBlob(L"HDR-ps.cso", &ps));
    ...
    // Create the SDR Root Signature
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed(D3DReadFileToBlob(L"HDRtoSDR-vs.cso", &vs));
        ThrowIfFailed(D3DReadFileToBlob(L"HDRtoSDR-ps.cso", &ps));
